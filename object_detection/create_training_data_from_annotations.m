function create_training_data_from_annotations(filepath, absolute_targetpath)
    ...
    try
        ...
        mkdir(fullfile(absolute_targetpath, 'JPEGImages'));
        mkdir(fullfile(absolute_targetpath, 'labels'));

        disp(['[+] ' fullfile(absolute_targetpath, 'JPEGImages') 'and ' fullfile(absolute_targetpath, 'labels') ' created!']);
    catch
        error('[-] Fail creating target folder');
    end

    imgpath = fullfile(absolute_targetpath, 'JPEGImages');
    labelpath = fullfile(absolute_targetpath, 'labels');

    coded_boxes = dir(fullfile(filepath, 'annotation_data_*.mat'));
    training_fid = fopen(fullfile(absolute_targetpath, 'training.txt'), 'w');

    if numel(coded_boxes) == 0
        error('[-] file does not exist/no annotation data found in the directory')
    else
        for i = 1:numel(coded_boxes)
            ...
            disp(oldImgPath2NewImgPath(annotation_data(1).frame_name));
