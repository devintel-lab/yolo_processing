function formattingCodedBoxes(filepath, targetpath)
if ~exist('filepath', 'var')
    filepath = fullfile(pwd, 'box_annotations');
end
if ~exist('targetpath', 'var')
    try
        mkdir training_data JPEGImages
        mkdir training_data labels
        disp('[+] /training_data/JPEGImages/ and /training_data/labels/ created')
    catch
        error('[-] Fail creating target folder')
    end
    targetpath = fullfile(pwd, 'training_data');
end
